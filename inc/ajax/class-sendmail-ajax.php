...
if ( ! class_exists( 'SendmailRunner' ) ) {
class SendmailRunner extends FtAjaxRunner {
	...
	protected function run() {
		...
		// ユーザー情報チェック
		if ( ( $chk_result = $this->chkUserInfo( $user_info, $fbid ) ) !== true ) {
			return array(
				'result' => 'error',
				'cause' => $chk_result
			);
	...
	private function chkUserInfo( $user, $fbid ) {
		// FBIDチェック
		if ( $fbid !== $user['fbid'] ) {
